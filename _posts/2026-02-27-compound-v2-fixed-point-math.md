...
The general term for this strategy of scaling integers to avoid decimal truncation is called fixed-point arithmetic. On-chain, only scaled values are used, so division never produces a truncated zero. Off-chain, divide by the scaling factor to get the true value.
...
Dividing by `1e18` to recover the original value gives `1e18`, not `1`. The scale is broken. The fix is to divide by the scaling factor once after multiplying two scaled values:
